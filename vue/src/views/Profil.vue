...
export default {
  ...
  methods: {
    updateProfile() {
      ...
      // console.log("forEach user_images")
        ...
        buttons: [
          {
            ...
            handler: () => {
              ...
              // console.log(`handleremove imagePRofil `)
                if (element.uid === file.uid) {
    ...
    UpdateProfil() {
      ...
      if (this.validate()) {
        ...
        let date = moment(old, "DD/MM/YYYY").format("YYYY-MM-DD");
        this.$http
          .put("users/", {
        ...
        if (!this.err) {
          if (this.dropFiles.length > 0 || this.imageProfil.length > 0) {
            ...
            // console.log(`forEach imageProfil ${JSON.stringify(element)}`)
              let image = new Image();
